...
>>>>>>> dd2badc00fe6c3bf7cf6483a6c898d5295da3d8e
...
puma_accidents <- nyc_point_poly(bike_accidents_location, "puma") %>%
  st_set_geometry(NULL) %>%
  group_by(year) %>%
  count(puma_name, puma_id) %>%
  rename(count = n)

bike_puma_accidents_final <- left_join(nyc_puma, puma_accidents)

bike_puma_accidents_final <- bike_puma_accidents_final %>% 
  filter(year=="2019" | year=="2020" | year=="2021")
  na.omit(puma_accidents)

ggplot() +
  geom_sf(data= bike_puma_accidents_final, aes(fill = count), size = 0.5) +
  theme(panel.background = element_rect(fill = "aliceblue")) +
  facet_grid(~ year)+ 
