...
To start, we wanted to know the location of both the bicycle trails and the bike accidents. Below, we compiled several different maps to show when, where, and why the accidents are happening. Upon getting results, we found an apparent pattern with several variables, such as path locations, year, accidents frequency, and contributing factors.
...
<!-- put space -->
<p>&nbsp;</p>
# Leaflet of Bicycle Trails and Accidents
This visualizes the current bicycle paths and accidents in the city, as reported by the NYC government. Manhattan, the borough with the most accidents, also has the most bicycle trails. Furthermore, as we go further out from the center of NYC, both the accidents and paths decrease. Thus, there must be a correlation with the concentration of trails to the amount of accidents that occur. 
```{r, echo = FALSE, message = FALSE, warning = FALSE}
...
bike_accidents_leaflet <- leaflet(bike_accidents_leaflet_sf) %>%
   ...
   addPolygons(fillColor = ~pal(bike_accidents_leaflet_sf$count), 
               color = "black", weight = 1, fillOpacity = 1, opacity = 0.5) %>% #dashArray = 2,
...
The graph below analyzes accident frequency during the months leading up to, before, and after the pandemic. Although the data differs between each year, there seems to be similarities with the overall shape, such as the increase in accidents after April as summer approaches and a decrease after September as it gets colder. 

Comparing 2020 to 2019, there were more overall accidents in 2020, specifically from June to October. Besides the approaching summer weather, June of 2020 was the peak of the pandemic. 

Though lockdown was implemented, this could have contributed to the increase in accidents since people were too afraid to use public transportation, such as cabs and the subway. As well as the increase in bicycle paths, following the "Green Wave". Furthermore, bicycle accidents between 2020 and 2021 decreased, and most likely due to COVID cases decreasing and people slowly starting to gain more confidence in using public transportation, once again. 
```{r, echo = FALSE, message = FALSE, warning=FALSE}
...
ggplot(accident_factors_19, mapping=aes(x = reorder(CONTRIBUTING.FACTOR.VEHICLE.1, count), y=count)) + 
  ...
  coord_flip() + 
  labs(
    title = str_wrap("Top 10 Contributing Factors of Accidents Involving Cyclists in 2019", 40),
    x = "Contributing Factors",
    y= "Total Count") +
    geom_text(aes(label = count), size = 3, hjust = 1.1, colour = "black")

ggplot(accident_factors_20, mapping=aes(x = reorder(CONTRIBUTING.FACTOR.VEHICLE.1, count), y=count)) + 
  geom_col(fill="light blue") + 
  coord_flip() + 
  labs(
    title = str_wrap("Top 10 Contributing Factors of Accidents Involving Cyclists in 2020", 40),
    x = "Contributing Factors",
    y= "Total Count") +
    geom_text(aes(label = count), size = 3, hjust = 1.1, colour = "black")
...
ggplot(accident_factors_21, mapping=aes(x = reorder(CONTRIBUTING.FACTOR.VEHICLE.1, count), y=count)) + 
  ...
  coord_flip() + 
  labs(
    title = str_wrap("Top 10 Contributing Factors of Accidents Involving Cyclists in 2021", 40),
    x = "Contributing Factors",
    y= "Total Count") +
    geom_text(aes(label = count), size = 3, hjust = 1.1, colour = "black")
